...
abstract class AbstractCategoryHelper
{
    ...
    public function retrieveCategoriesFromRequest($objectType = '', $source = 'POST')
    {
        ...
        $inputValues = null;
        $inputName = 'munewsmodule_' . strtolower($objectType) . 'quicknav';
        if (!$dataSource->has($inputName)) {
            $inputName = 'munewsmodule_' . strtolower($objectType) . 'finder';
        }
        if ($dataSource->has($inputName)) {
     ...
     */
    public function hasPermission(EntityAccess $entity)
    {
     ...
     */
    protected function requireAccessForAll(EntityAccess $entity)
    {
        return false;
    }
}
