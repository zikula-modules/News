 ...
 */
class MUNews_Controller_Base_External extends Zikula_AbstractController
{
    /**
    ...
    public function display(array $args = array())
    {
        ...
        $idFields = ModUtil::apiFunc('MUNews', 'selection', 'getIdFields', array('ot' => $objectType));
    ...
    {
        PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('MUNews'));
    
        $getData = $this->request->query;
        $controllerHelper = new MUNews_Util_Controller($this->serviceManager);
    
        $objectType = $getData->filter('objectType', 'message', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'external', 'action' => 'finder');
        if (!in_array($objectType, $controllerHelper->getObjectTypes('controller', $utilArgs))) {
            $objectType = $controllerHelper->getDefaultObjectType('controllerType', $utilArgs);
        }
    
        $this->throwForbiddenUnless(SecurityUtil::checkPermission('MUNews:' . ucwords($objectType) . ':', '::', ACCESS_COMMENT), LogUtil::getErrorMsgPermission());
        ...
        $categoryIds = ModUtil::apiFunc('MUNews', 'category', 'retrieveCategoriesFromRequest', array('ot' => $objectType, 'source' => 'GET'));
        ...
        $view = Zikula_View::getInstance('MUNews', false);
