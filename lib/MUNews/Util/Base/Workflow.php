 ...
 */
class MUNews_Util_Base_Workflow extends Zikula_AbstractBase
{
    /**
    ...
    public function getActionsForObject($entity)
    {
        ...
        $listHelper = new MUNews_Util_ListEntries($this->serviceManager);
        ...
        if (SecurityUtil::checkPermission($modname . ':' . ucwords($objectType) . ':', '::', ACCESS_ADD)) {
            ...
            if ($amount > 0) {
                $amounts[] = array(
                    ...
                    'description' => $this->__('Messages pending approval'),
                    'amount' => $amount,
                    'objectType' => $objectType,
                    'state' => $state,
                    'message' => $this->_fn('One message is waiting for approval.', '%s messages are waiting for approval.', $amount, array($amount))
                );
            }
        }
