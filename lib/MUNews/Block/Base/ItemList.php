 ...
 */
class MUNews_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    /**
    ...
    {
        SecurityUtil::registerPermissionSchema('MUNews:ItemListBlock:', 'Block title::');
    
        $this->categorisableObjectTypes = array('message');
    }
    ...
    public function info()
    {
        ...
        if (!ModUtil::available('MUNews')) {
            $requirementMessage .= $this->__('Notice: This block will not be displayed until you activate the MUNews module.');
        }
    
        return array('module'          => 'MUNews',
                     'text_type'       => $this->__('MUNews list view'),
                     'text_type_long'  => $this->__('Display list of MUNews objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (!SecurityUtil::checkPermission('MUNews:ItemListBlock:', "$blockinfo[title]::", ACCESS_OVERVIEW)) {
            return false;
        }
        ...
        if (!isset($vars['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('MUNews', 'category', 'getPrimaryProperty', array('ot' => $vars['objectType']));
        ...
        $entityClass = 'MUNews_Entity_' . ucwords($objectType);
        ...
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('MUNews', 'category', 'getAllProperties', array('ot' => $objectType));
        }
    
        // apply category filters
        if (in_array($objectType, $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $qb = ModUtil::apiFunc('MUNews', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $objectType, 'catids' => $vars['catIds']));
            }
        }
        ...
        if (empty($blockinfo['title'])) {
            $blockinfo['title'] = $this->__('MUNews items');
        }
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('MUNews', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $vars['objectType'] = $this->request->request->filter('objecttype', 'message', FILTER_SANITIZE_STRING);
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('MUNews', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
