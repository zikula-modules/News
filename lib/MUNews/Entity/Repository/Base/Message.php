 ...
 */
class MUNews_Entity_Repository_Base_Message extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $listHelper = new MUNews_Util_ListEntries(ServiceUtil::getManager());
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('message', 'workflowState');
                $booleanSelectorItems = array(
                    array('value' => 'no', 'text' => __('No')),
                    array('value' => 'yes', 'text' => __('Yes'))
                );
                $templateParameters['noEndDateItems'] = $booleanSelectorItems;
            }
    
            // initialise Imagine preset instances
            $imageHelper = new MUNews_Util_Image(ServiceUtil::getManager());
    
            $objectType = 'message';
            $templateParameters[$objectType . 'ThumbPresetImageUpload1'] = $imageHelper->getPreset($objectType, 'imageUpload1', $context, $args);
            $templateParameters[$objectType . 'ThumbPresetImageUpload2'] = $imageHelper->getPreset($objectType, 'imageUpload2', $context, $args);
            $templateParameters[$objectType . 'ThumbPresetImageUpload3'] = $imageHelper->getPreset($objectType, 'imageUpload3', $context, $args);
            $templateParameters[$objectType . 'ThumbPresetImageUpload4'] = $imageHelper->getPreset($objectType, 'imageUpload4', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'MUNews_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('MUNews', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'message', 'source' => 'GET'));
        ...
        $parameters['noEndDate'] = isset($this->controllerArguments['noEndDate']) ? $this->controllerArguments['noEndDate'] : FormUtil::getPassedValue('noEndDate', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('MUNews_Entity_Message', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('MUNews_Entity_Message', 'tbl')
     ...
     *
     * @return array|MUNews_Entity_Message retrieved data array or MUNews_Entity_Message instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     *
     * @return MUNews_Entity_Message retrieved instance of MUNews_Entity_Message
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved MUNews_Entity_Message instances
     */
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('MUNews', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'message', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
                    $qb = $this->addSearchFilter($qb, $v);
                }
            } elseif (in_array($k, array('noEndDate'))) {
                // boolean filter
                if ($v == 'no') {
                    $qb->andWhere('tbl.' . $k . ' = 0');
                } elseif ($v == 'yes' || $v == '1') {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentType == 'admin' && $currentModule == 'MUNews') {
            return $qb;
        }
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            ...
            $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('MUNews', 'showOnlyOwnEntries', 0), 'GETPOST');
            if ($showOnlyOwnEntries == 1) {
                // allow the owner to see his deferred messages
    ...
    protected function addSearchFilter(QueryBuilder $qb, $fragment = '')
    {
        ...
        if (!$fragmentIsNumeric) {
            ...
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.startText LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.imageUpload1 = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.mainText LIKE \'%' . $fragment . '%\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.imageUpload2 = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.imageUpload3 = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.imageUpload4 = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            ...
            $where .= 'tbl.imageUpload4 = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.muimageAlbum = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.weight = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.startDate = \'' . $fragment . '\'';
            $where .= ((!empty($where)) ? ' OR ' : '');
            $where .= 'tbl.endDate = \'' . $fragment . '\'';
     ...
     * @param int    $excludeId  Id of messages to exclude (optional).
     *
    ...
    {
        if (PageUtil::getVar('MUNewsAutomaticArchiving', false) !== true && !SecurityUtil::checkPermission('MUNews', '.*', ACCESS_EDIT)) {
        ...
        foreach ($affectedEntities as $entity) {
            ...
            ModUtil::apiFunc('MUNews', 'cache', 'clearItemCache', $cacheArgs);
        }
