 ...
 */
class MUNews_Base_RouterFacade
{
    /**
    ...
    protected function initUrlRoutes()
    {
        ...
        $isDefaultModule = (System::getVar('shorturlsdefaultmodule', '') == 'MUNews');
    
        $defaults = array();
        $modulePrefix = '';
        if (!$isDefaultModule) {
            $defaults['module'] = 'MUNews';
            $modulePrefix = ':module/';
        }
    ...
    public function getFormattedSlug($objectType, $func, $args, $itemid)
    {
        ...
        switch ($objectType) {
            case 'message':
                $item = ModUtil::apiFunc('MUNews', 'selection', 'getEntity', array('ot' => $objectType, 'id' => $itemid, 'slimMode' => true));
                $slug = $item['slug'];
                        break;
        }
