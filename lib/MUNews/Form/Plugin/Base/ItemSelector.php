 ...
 */
class MUNews_Form_Plugin_Base_ItemSelector extends Zikula_Form_Plugin_TextInput
{
    /**
    ...
    public function render(Zikula_Form_View $view)
    {
        ...
        if ($firstTime) {
            ...
            PageUtil::addVar('javascript', 'modules/MUNews/javascript/MUNews_finder.js');
            PageUtil::addVar('stylesheet', ThemeUtil::getModuleStylesheet('MUNews'));
        }
        $firstTime = false;

        if (!SecurityUtil::checkPermission('MUNews:' . ucwords($this->objectType) . ':', '::', ACCESS_COMMENT)) {
            return false;
        }

        $categorisableObjectTypes = array('message');
        $catIds = array();
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            ...
            $catIds = ModUtil::apiFunc('MUNews', 'category', 'retrieveCategoriesFromRequest', array('ot' => $this->objectType));
        }
        ...
        $properties = null;
        if (in_array($this->objectType, $categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('MUNews', 'category', 'getAllProperties', array('ot' => $this->objectType));
        }
    ...
    {
        parent::decode($view);
        $this->objectType = FormUtil::getPassedValue('MUNews_objecttype', 'message', 'POST');
        $this->selectedItemId = $this->text;
    }
}
