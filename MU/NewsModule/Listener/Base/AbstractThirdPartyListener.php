...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        if (!$this->filesystem->exists($targetDir)) {
            ...
            if (is_dir($originDir = 'modules/MU/NewsModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/MU/NewsModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                'module' => 'MUNewsModule',
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/munews/js/MUNewsModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'munewsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/munews/scribite/CKEditor/munewsmodule/',
            'file' => 'plugin.js',
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'munewsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/munews/scribite/Quill/munewsmodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'munewsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/munews/scribite/Summernote/munewsmodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'munewsmodule',
            'path' => $this->request->getBasePath() . '/web/modules/munews/scribite/TinyMce/munewsmodule/plugin.js'
        ]);
    }
}
