...
abstract class AbstractUploadHelper
{
    ...
    public function __construct(
        ...
        $this->moduleVars = $variableApi->getAll('MUNewsModule');
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'MUNewsModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'MUNewsModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
    ...
    {
        $basePath = $this->dataDirectory . '/MUNewsModule/';
        ...
        switch ($objectType) {
            case 'message':
                ...
                if ('' != $fieldName) {
                    switch ($fieldName) {
                        case 'imageUpload1':
                            $basePath .= 'imageupload1/';
                            break;
                        case 'imageUpload2':
                            $basePath .= 'imageupload2/';
                            break;
                        case 'imageUpload3':
                            $basePath .= 'imageupload3/';
                            break;
                        case 'imageUpload4':
                            $basePath .= 'imageupload4/';
                            break;
                    }
                }
                break;
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$this->filesystem->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'MUNewsModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'MUNewsModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/MU/NewsModule/Resources/docs/htaccessTemplate';
        if (!$this->filesystem->exists($htaccessFilePath) && $this->filesystem->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'MUNewsModule', 'directory' => $uploadPath]);
